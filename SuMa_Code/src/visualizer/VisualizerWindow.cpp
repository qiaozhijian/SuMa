...
VisualizerWindow::VisualizerWindow() {
    ...
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("current points", value);
    });
    connect(ui_.chkShowCoordinateAxis, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("coordinate axis", value);
    });
    ...
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("normal map", value);
    });
    connect(ui_.chkShowDepthImage, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("depth map", value);
    });
    connect(ui_.chkShowVertexMap3d, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("vertex map", value);
    });
    connect(ui_.chkShowNormalMap3d, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("normal map 3d", value);
    });
    connect(ui_.chkMarkInvalidDepthValues, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("mark invalid depths", value);
    });
    connect(ui_.chkShowCurrentSurfels, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("current surfels", value);
    });
    connect(ui_.chkShowIcpEstimate, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("pose estimate", value);
    });

    connect(ui_.chkShowLastFrame, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("last frame", value);
    });

    connect(ui_.chkShowGroundtruth, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("groundtruth", value);
    });

    connect(ui_.chkShowPoses, &QCheckBox::toggled, [=](bool value) {
        ui_.wCanvas->setDrawingOption("poses", value);
    });

    connect(ui_.chkFollowPose, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("follow pose", value);
    });

    connect(ui_.chkShowHistory, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("draw history", value);
    });

    connect(ui_.chkShowUpdatedSurfels, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("updated surfels", value);
    });

    connect(ui_.chkBackfaceCulling, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("backface culling", value);
    });

    connect(ui_.chkShowSubmaps, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("show submaps", value);
    });

    connect(ui_.chkShowRobot, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("draw robot", value);
    });
    ...
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("pose graph", value);
    });

    connect(ui_.chkShowCandidates, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("show lc candidates", value);
    });
    ...
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("old map only", value);
    });

    connect(ui_.chkColorizeHistory, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("colorize history", value);
    });

    connect(ui_.chkShowLoopPoses, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("show lc poses", value);
    });
    ...
    connect(ui_.btnResidualPlot, &QPushButton::clicked, [=](bool value) {
        fusion_->genResidualPlot();
    });
    connect(ui_.chkShowGraphWidget, &QCheckBox::toggled, [=](bool value) {
        ui_.wGraph->setVisible(value);
    });

    connect(ui_.chkShowOdomPoses, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("draw odom poses", value);
    });

    connect(ui_.chkBirdsEyeView, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("birds eye view", value);
    });

    connect(ui_.chkRemoveGround, &QCheckBox::toggled,
    [=](bool value) {
        ui_.wCanvas->setDrawingOption("remove ground", value);
    });
...
//   固定读入类型
    QString retValue =
        QFileDialog::getOpenFileName(this, "Select laserscan file", lastDirectory_,
                                     "Laserscan files(*.bin *.pcap *.sim *.pcd);;KITTI laserscans (*.bin *.pcd);; PCAP "
...
void VisualizerWindow::initialize(const std::shared_ptr<SurfelMapping>& fusion, const ParameterList& params) {
    ...
    // get parameters.把default中的参数在窗口上显示
...
void VisualizerWindow::startRecording() {
    if (!ui_.actionRecord->isChecked()) {
        ...
        QString retValue = QFileDialog::getExistingDirectory(this, "Select output directory", lastDirectory_);//保存路径
        if (!retValue.isNull()) {
