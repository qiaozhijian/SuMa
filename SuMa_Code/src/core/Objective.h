...
public:
    ...
    uint32_t valid() const {
        return (inlier_ + outlier_);
    }
    uint32_t invalid() const {
        return invalid_;
    }

    float inlier_residual() const {
        return inlier_residual_;
    }

    /** \brief initialize the pose estimate. **/
    void initialize(const Eigen::Matrix4d& T0) {
        pose_ = T0;
    }
    ...
    virtual uint32_t getMaxLevel() const {
        return 0;
    }
