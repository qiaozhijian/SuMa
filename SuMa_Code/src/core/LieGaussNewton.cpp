...
int32_t LieGaussNewton::minimize(Objective& F, const Eigen::Matrix4d& T0) {//T0初始值为单位矩阵
    history_.clear();

    initialize(F, T0);//这里Tk_=T0
...
//    std::cout << " ==== LEVEL " << i << " ==== " << std::endl;
        for (;;) {
            history_.push_back(Tk_);//Tk_是最优位姿，k是迭代次数
...
int32_t LieGaussNewton::step() {
    ...
    if (std::abs(Jtf.maxCoeff()) < epsilon) result = 0;                                            // gradient.梯度 maxcoeff最大元素
    if (current_error < last_error && std::abs(current_error - last_error) < epsilon) result = 0;  // converged.小于某个阈值认为收敛，返回值为0
