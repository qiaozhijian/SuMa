...
void SurfelMapping::savePose() {
    std::ofstream out;
    out.open("/home/lvsijie/myfile.txt");
    if(!out) {
    ...
    for (uint32_t i = 0; i < poses.size(); ++i) {
        ...
        Eigen::Quaterniond q =Eigen::Quaterniond(R);//得到四元数
        for (uint32_t r = 0; r < 3; ++r) {
            for (uint32_t c = 3; c < 4; ++c) {
                //out << i << " " << ((r == 0 && c == 0) ? "" : " ") << pose(r, c);//只输出平移向量
                out << i << " " << pose(r, c);//只输出平移向量
            }
        }
        out << " " << q.x();
        out << " " << q.y();
