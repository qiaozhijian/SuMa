      ...
      fbo_blend_(2, 8)  // blending for matrix comp.
{
    ...
    for (uint32_t i = 0; i < width; i += entriesPerKernel_) {
        for (uint32_t j = 0; j < height; ++j) {
            img_coords.push_back(vec2(i + 0.5f, j + 0.5f));//建立坐标点
        }
    }

    vbo_img_coords_.assign(img_coords);//缓存对象中存放img_coords的数据
    vao_img_coords_.bind();//选定这个数组对象
    vao_img_coords_.setVertexAttribute(0, vbo_img_coords_, 2, AttributeType::FLOAT, false, 2 * sizeof(float), nullptr);//把各信息关联上
...
void Frame2Model::updateParameters() {//读取default文件中的数据
...
double Frame2Model::jacobianProducts(Eigen::MatrixXd& JtJ, Eigen::MatrixXd& Jtf) {
    ...
    glPointSize(1.0f);//指定栅格化点的直径，即图上的点云
