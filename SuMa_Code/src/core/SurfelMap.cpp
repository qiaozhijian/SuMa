...

{
    ...
    img_coords.reserve(dataWidth_ * dataHeight_);//把img_coords向量的容量设置为dataWidth_ * dataHeight_那么长

    for (uint32_t x = 0; x < dataWidth_; ++x) {
        for (uint32_t y = 0; y < dataHeight_; ++y) {
            img_coords.push_back(vec2(x + 0.5f, y + 0.5f));//把坐标(x + 0.5f, y + 0.5f)压入img_coords中
        }
    }

    vbo_img_coords_.assign(img_coords);//拷贝数据
    ...
    glDrawBuffers(3, buffs3);//在buff3中指定3个颜色缓冲区
...
void SurfelMap::setParameters(const rv::ParameterList& params) {
    ...
    float hfov = 360.0;
    // which is the tangens of the half per-pixel angular size...计算pixel大小
    float vpix = std::tan(0.5f * Math::deg2rad(vfov) / uint32_t(params["data_height"])) * 1.0f;//height像是 高 上的像素个数
    float hpix = std::tan(0.5f * Math::deg2rad(hfov) / uint32_t(params["data_width"])) * 1.0f;
    float pixel_size = std::max(vpix, hpix);//像素尺寸
...
void SurfelMap::update(const Eigen::Matrix4f& pose, Frame& frame) {
    ...
    glDepthFunc(GL_LESS);//输入的深度值小于参考值则通过
    ...
    renderIndexmap(pose, inv_pose);//inv_pose=pose.inverse()
...
void SurfelMap::renderIndexmap(const Eigen::Matrix4f& pose, Eigen::Matrix4f& inv_pose) {
    ...
    indexMapFramebuffer_.bind();//绑定帧缓存
    indexMap_program_.bind();//绑定GlProgram
    ...
    glDrawArrays(GL_POINTS, 0, surfels_.size());//把vao_surfels_数组中的各点以点的形式连接
...
void SurfelMap::generateDataSurfels(Frame& frame) {
    glViewport(0, 0, radiusConfidenceFramebuffer_.width(), radiusConfidenceFramebuffer_.height());//画一个视图（左下角坐标，长宽）
    ...
    glDrawArrays(GL_POINTS, 0, vbo_img_coords_.size());//（以点的形式，从数组的第几个元素开始，顶点个数）
    vao_img_coords_.release();
...
void SurfelMap::draw(const Eigen::Matrix4f& modelview, const SurfelMapVisualOptions& params) {
    ...
    float psize;
    glGetFloatv(GL_POINT_SIZE, &psize);//获得surfel的半径
    ...
    if (params.drawPoints) {
        ...
        glPointSize(2.0f * psize);//转化为点的直径
    }
