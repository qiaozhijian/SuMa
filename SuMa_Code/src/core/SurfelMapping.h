...
public:
    ...
    Frame::Ptr getOldSurfelMap() {
        return map_->oldMapFrame();
    }
    Frame::Ptr getNewSurfelMap() {
        return map_->newMapFrame();
    }
    ...
    const std::vector<Eigen::Matrix4d>& getIntermediateOdometryPoses() const {
        return odom_poses_;
    }
...
protected:
    ...
    Eigen::DiagonalMatrix<double, 6> info_;//对角矩阵
